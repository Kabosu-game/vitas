...
/**
 * Script de mise à jour des traductions de la navigation utilisateur
 * Usage : php update_nav_translations.php
 */
...
foreach ($translations as $type => $trans) {
    $rows = DB::table('user_navigations')->where('type', $type)->get();
    foreach ($rows as $row) {
        DB::table('user_navigations')->where('id', $row->id)->update([
            'translation' => json_encode($trans),
            'updated_at'  => now(),
        ]);
        $updated++;
        echo "Mis à jour : {$type}\n";
    }
}

echo "\nTerminé — {$updated} éléments mis à jour.\n";
