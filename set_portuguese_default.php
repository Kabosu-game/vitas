...
/**
 * Script pour définir le portugais comme langue par défaut
 * Usage : php set_portuguese_default.php
 */
...
echo "=== Définir le portugais comme langue par défaut ===\n\n";

// 1. Vérifier si la langue portugaise existe
try {
    $portuguese = DB::table('languages')->where('locale', 'pt')->first();
    
    if (!$portuguese) {
        echo "ERREUR: La langue portugaise (pt) n'existe pas dans la base de données.\n";
        echo "Veuillez d'abord exécuter add_portuguese_language.php\n";
        exit(1);
    }
    
    if (!$portuguese->status) {
        echo "ERREUR: La langue portugaise existe mais est désactivée.\n";
        echo "Veuillez d'abord l'activer avec add_portuguese_language.php\n";
        exit(1);
    }
    
    echo "1. Langue portugaise trouvée et active.\n";
} catch (Exception $e) {
...
// 3. Définir le portugais comme langue par défaut
try {
    DB::table('languages')
        ->where('locale', 'pt')
        ->update(['is_default' => 1, 'updated_at' => now()]);
    
    echo "3. Português (pt) défini comme langue par défaut.\n";
} catch (Exception $e) {
...
try {
    ...
    if ($newDefault && $newDefault->locale === 'pt') {
        echo "4. SUCCESS: Le portugais est maintenant la langue par défaut.\n";
    } else {
...
echo "\n7. Notes importantes:\n";
echo "   - Le site s'affichera maintenant en portugais par défaut\n";
echo "   - Les nouveaux visiteurs verront le site en portugais\n";
echo "   - Les utilisateurs existants garderont leur langue préférée\n";
echo "   - Le sélecteur de langue permet toujours de changer\n";

echo "\n=== Opération terminée ===\n";
echo "Le portugais est maintenant la langue par défaut du site.\n";
