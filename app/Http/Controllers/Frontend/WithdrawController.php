...
class WithdrawController extends Controller
{
    ...
    public function details($accountId, int $amount = 0)
    {
        ...
        $processingTime = (int) $method->required_time > 0 ? __('Processing Time: ') . $withdrawAccount->method->required_time . $withdrawAccount->method->required_time_format : __('This Is Automatic Method');
    ...
    public function withdrawBank(Request $request)
    {
        ...
        $bankDetails = [
            'Titulaire du compte' => ['type' => 'text', 'value' => $request->holder_name],
            'IBAN'                => ['type' => 'text', 'value' => $request->iban],
            'Banque'              => ['type' => 'text', 'value' => $request->bank_name],
        ];
        if ($request->filled('note')) {
            $bankDetails['Remarque'] = ['type' => 'text', 'value' => $request->note];
        }
        ...
        $txnInfo = Txn::new(
            ...
            $amount,
            'Virement bancaire',
            'Demande de retrait par virement',
            TxnType::Withdraw,
            TxnStatus::Pending,
        ...
        $shortcodes = [
            ...
            '[[withdraw_amount]]'=> $amount.' '.setting('site_currency', 'global'),
            '[[site_title]]'     => setting('site_title', 'global'),
            '[[site_url]]'       => route('home'),
        ];
