...
trait Payment
{
    ...
    protected function withdrawAutoGateway($gatewayCode, $txnInfo, $isApi = false)
    {
        $gatewayTxn = self::gatewayMap($gatewayCode, $txnInfo);

        if ($gatewayTxn && config('app.demo') == 0) {
            $withdrawResponse = $gatewayTxn->withdraw();
            if (!$withdrawResponse) {
                return back();
            } elseif ($withdrawResponse instanceof \Illuminate\Http\RedirectResponse) {
